        ...
        if cid==-1:
            ...
            try:
                if status == 'train':
                    ...
                    sent = TOKENIZER.encode(sent, add_special_tokens=True)
                    sent = [sent[0]] + [TOKENIZER.unk_token_id] + sent[1:-1] + [TOKENIZER.unk_token_id] + [sent[-1]] # 1. UNK for definition; 2. UNK for concept
                ...
                if status == 'train':
                    sent = f"{definition} {question.lower()}."
                    sent = TOKENIZER.encode(sent, add_special_tokens=True)
                    sent = sent[:-1] + [TOKENIZER.unk_token_id] + [sent[-1]] #1. UNK for concept
